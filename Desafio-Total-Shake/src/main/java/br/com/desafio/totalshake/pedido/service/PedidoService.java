package br.com.desafio.totalshake.pedido.service;
...
import br.com.desafio.totalshake.pedido.repository.PedidoRepository;
...
public class PedidoService {
    ...
    public Page<PedidoDto> getAllPedidos(Pageable pageable) throws NaoEncontradoException {

        Page<Pedido> pedidoModel;

        if(!repository.findAll().isEmpty()){

            pedidoModel = repository.findAll(pageable);

            Page<PedidoDto> listaPedidos =
                    modelMapper.map(pedidoModel, new TypeToken<Page<PedidoDto>>() {}.getType());
